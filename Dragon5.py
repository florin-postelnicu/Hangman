    ...
    while current_room_index < len(rooms):
        ...
        if current_room.choices:
            ...
            for i, choice in enumerate(current_room.choices, 1):
                print(f"{i}. {choice}")
                print("Enter your choice (1 - {}): ".format(len(current_room.choices))))
                print("You chose: {}".format(current_room.choices[choice_index - 1]))
                # Apply gains for the player
                if current_room.gain:
                    for
                attribute, value in current_room.gain.items():
