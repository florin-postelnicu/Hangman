...
def generate_chatbot_prompt(current_room):
    # Generate a prompt based on the current room's description, choices, or other relevant information
    prompt = f"In the current room, you encounter: {current_room.description}\n"

    if current_room.choices:
        prompt += "You have the following choices:\n"
        for i, choice in enumerate(current_room.choices, 1):
            prompt += f"{i}. {choice}\n"

    if current_room.puzzle:
        puzzle_description, _ = current_room.puzzle
        prompt += f"There is a puzzle to solve: {puzzle_description}\n"

    # Add any additional context or prompts specific to your game

    return prompt
# Define game loop
# Define game loop
# Define game loop
def play_game(player, rooms, chatbot):
    ...
    while current_room_index < len(rooms):
        ...
        # Interact with ChatGPT for hints or guidance
        chatbot_prompt = generate_chatbot_prompt(current_room)
        chatbot_response = chatbot.send_prompt(chatbot_prompt)
        print("ChatGPT: {}".format(chatbot_response))
